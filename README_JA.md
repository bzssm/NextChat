...
### `NON_VISION_MODELS` (オプション)

> デフォルト：空
> 例：`gpt-3.5-turbo,my-text-only-model` は、これらのモデルの画像入力を無効にします。

デフォルトではすべてのモデルで画像入力が有効です。画像入力を無効にするモデル名をカンマで区切って指定します。各項目の前後の空白を除去し、大文字・小文字を区別してモデル名全体と完全一致で判定します。空の項目は無視されます。画像を含むリクエストが成功するには、モデルサービス自体が画像入力に対応している必要があります。

この設定は `VISION_MODELS` を置き換えます。旧環境変数と組み込みのビジョンモデル正規表現は、この判定には使用されません。
