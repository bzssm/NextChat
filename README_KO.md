...
### `NON_VISION_MODELS` (선택 사항)

> 기본값: 비어 있음  
> 예시: `gpt-3.5-turbo,my-text-only-model`은 해당 모델의 이미지 입력을 비활성화합니다.

기본적으로 모든 모델에서 이미지 입력이 활성화됩니다. 이미지 입력을 비활성화할 모델 이름을 쉼표로 구분하세요. 각 항목의 양쪽 공백을 제거한 뒤 대소문자를 구분하여 전체 모델 이름과 정확히 일치하는지 확인하며, 빈 항목은 무시합니다. 이미지가 포함된 요청이 성공하려면 모델 서비스 자체가 이미지 입력을 지원해야 합니다.

이 설정은 `VISION_MODELS`를 대체합니다. 이전 환경 변수와 내장 시각 모델 정규식은 이 판정에 더 이상 사용되지 않습니다.
